...
export default function LoginPage() {
  ...
  return <div className="mx-auto max-w-md rounded-2xl border border-slate-200 bg-white p-6 space-y-5 shadow-sm">
    ...
    {email ? <>
      ...
      <button type="button" onClick={() => signOut({ callbackUrl: '/' })} className="rounded-lg border border-slate-300 px-4 py-2 text-sm">登出</button>
    </> : <>
      <p className="text-sm text-slate-600">沒有設定登入平台時，可繼續以此瀏覽器的匿名身分收藏商品。匿名收藏不會自動轉移至其他裝置或登入帳號。</p>
      <div className="space-y-2">
        {providers.map((provider) => <button key={provider.id} type="button" onClick={() => signIn(provider.id, { callbackUrl: '/favorites' })} className="block w-full rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700">使用 {provider.name} 登入</button>)}
      </div>
